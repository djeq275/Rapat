...
- AC: Halaman tambah/edit grup Telegram menampilkan daftar Chat ID yang terdeteksi otomatis (dari Telegram Bot API `getUpdates`) untuk dipilih, sebagai bantuan pengisian -- bukan pengganti input manual; grup yang Chat ID-nya tidak muncul di daftar tetap harus bisa diisi manual seperti sekarang.
- AC: Daftar Chat ID otomatis **tidak ditampilkan sama sekali** jika token bot Telegram belum disetel (mengikuti pola pengecekan token yang sudah ada di pengaturan bot) -- bukan ditampilkan kosong atau menampilkan pesan error; form tetap berfungsi dengan input manual seperti sekarang.
...
**US-11 — Admin mengelola role kustom & hak aksesnya**
Sebagai Admin, saya ingin membuat role baru selain 4 role bawaan (Admin/Direktur/Ketua Divisi/Karyawan) dan mengatur hak akses (halaman/aksi apa saja yang bisa diakses) untuk role tersebut, supaya struktur organisasi yang lebih bervariasi bisa direpresentasikan tanpa menunggu perubahan kode setiap kali ada kebutuhan akses baru.
- AC: Admin bisa membuat, mengubah nama, dan menghapus role kustom. 4 role bawaan (Admin, Direktur, Ketua Divisi, Karyawan) tidak bisa dihapus atau diubah namanya, karena masing-masing punya perilaku bawaan yang di-hardcode di aplikasi (lihat Architecture Overview).
- AC: Admin bisa mencentang hak akses per role kustom dari daftar kapabilitas yang tersedia (mis. kelola pengguna, kelola divisi, kelola grup Telegram, kelola konfigurasi Keycloak/Google OAuth) — granularitas per-fitur/menu, bukan per-baris data.
- AC: Admin bisa mencentang kapabilitas perilaku tambahan per role dari 3 pilihan berikut, berlaku juga untuk role kustom (nilai untuk role bawaan sudah di-seed sesuai perilaku yang berlaku hari ini): "Auto-invite ke semua rapat" (perilaku Direktur), "Lihat rapat dari semua divisi" (perilaku Direktur/Admin), "Bisa jadi organizer rapat & manager notulensi untuk divisinya" (perilaku Ketua Divisi) — perilaku ini tidak lagi eksklusif milik 2 role bawaan tersebut.
- AC: Ketiga flag perilaku di atas independen satu sama lain — Admin bisa memberi role kustom kombinasi apa pun (mis. role "Wakil Direktur" yang auto-invite + lihat semua divisi tapi bukan organizer; atau role "Manajer Regional" yang organizer tapi tidak auto-invite).
- AC: Nilai flag perilaku untuk 4 role bawaan (Admin, Direktur, Ketua Divisi, Karyawan) sudah diseed sesuai perilaku yang berlaku hari ini dan tidak berubah oleh perubahan ini — perubahan ini murni memindahkan pemeriksaan role-by-name di kode jadi pemeriksaan flag di data, tanpa mengubah siapa bisa apa hari ini.
- AC: User dengan role kustom yang tidak diberi hak akses maupun flag perilaku apa pun hanya bisa mengakses halaman yang memang terbuka untuk semua user login (analog dengan Karyawan tanpa divisi hasil auto-provisioning Keycloak, lihat US-1).
- AC: Hanya Admin yang bisa membuat/mengubah role dan hak aksesnya — tidak ada delegasi pengelolaan role ke role lain pada rilis ini.
...
- **Bantuan pemilihan Chat ID grup Telegram**: saat Admin membuka halaman tambah/edit grup Telegram dan token bot sudah disetel, aplikasi memanggil Telegram Bot API `getUpdates` untuk memperoleh daftar Chat ID yang baru-baru ini berinteraksi dengan bot, lalu menampilkannya sebagai pilihan di form (Chat ID + nama grup/pengirim jika tersedia dari payload update) -- field Chat ID tetap bisa diisi/diubah manual sebagai alternatif. Jika token bot belum disetel, daftar ini tidak ditampilkan sama sekali dan form tetap berfungsi seperti sekarang (input manual).
...
- **Role kustom & hak akses (US-11)**: 4 role bawaan (Admin, Direktur, Ketua Divisi, Karyawan) tetap ada sebagai baris "terlindungi" yang tidak bisa dihapus/diubah nama, karena masing-masing punya perilaku bawaan (lihat flag perilaku di bawah) yang nilainya sudah ditetapkan sesuai peran masing-masing. Fitur ini **menambah** lapisan role kustom di atas 4 role bawaan itu (bukan menggantikan/menggeneralisasi semuanya), dengan hak akses per role kustom berupa daftar kapabilitas/menu yang bisa di-assign Admin. Ini menggeser sebagian pemeriksaan akses dari role hardcode di kode (`@PreAuthorize("hasRole(...)")`) menjadi hak akses yang dibaca dari database saat runtime.
- **Flag perilaku role (US-11)**: selain hak akses halaman/menu, setiap role (bawaan maupun kustom) juga punya 3 flag perilaku independen yang dibaca dari data, bukan lagi dari nama role secara hardcode — "auto-invite ke semua rapat" (sebelumnya eksklusif Direktur, lihat US-3), "lihat rapat dari semua divisi" (sebelumnya eksklusif Direktur/Admin), dan "bisa jadi organizer rapat & manager notulensi untuk divisinya" (sebelumnya eksklusif Ketua Divisi/Admin, lihat US-2 dan US-5). Nilai ketiga flag ini untuk 4 role bawaan di-seed identik dengan perilaku yang berlaku hari ini, sehingga tidak ada perubahan yang terlihat pengguna kecuali Admin secara sadar meng-assign flag yang sama ke role kustom. Flag "bisa jadi organizer" ini **terpisah** dari `Division.ketuaDivisiUserId` (satu user spesifik yang jadi kepala satu divisi tertentu, sebuah identitas per-divisi) — seorang user hanya benar-benar bisa mengorganisir rapat kalau role-nya punya flag ini, terlepas dari apakah dia tercatat sebagai kepala divisi tersebut atau tidak.
...
- **Role kustom & hak aksesnya (US-11)** adalah data sensitif dari sisi keamanan — kesalahan konfigurasi bisa membuka akses ke fitur admin yang tidak semestinya. Hanya Admin yang boleh membuat/mengubah role dan hak aksesnya (tidak ada self-service); hak akses berbasis role kustom ini dibatasi pada kapabilitas/menu tingkat fitur (siapa bisa mengakses halaman apa), bukan menggantikan izin granular per-rapat (notulis) yang sudah sengaja dipisah dari RBAC di atas.
...
- **Prasyarat manual bot Telegram**: bot harus sudah ditambahkan sebagai member/admin di setiap grup tujuan — prasyarat operasional ini tidak bisa dihilangkan. Risiko salah input Chat ID **dikurangi** (bukan dihilangkan total) oleh daftar Chat ID otomatis dari `getUpdates` di form tambah/edit grup, tapi bantuan ini tetap punya keterbatasan: `getUpdates` hanya mengembalikan update yang belum "dikonsumsi" dan tidak disimpan Telegram selamanya, jadi grup yang baru ditambahkan bot dan belum ada aktivitas pesan apa pun mungkin belum muncul di daftar sampai ada pesan baru dikirim ke grup tersebut — Admin tetap perlu opsi input manual sebagai fallback.
- **Rate limit `getUpdates` pada form grup Telegram**: mirip rate limit pengiriman pesan (lihat poin berikutnya), pemanggilan `getUpdates` sebaiknya tidak dilakukan tanpa batas di setiap render halaman form.
...
- **Migrasi dari pemeriksaan role hardcode ke hak akses dinamis (US-11)**: hampir seluruh pemeriksaan akses saat ini adalah string role yang dikompilasi langsung di kode (`@PreAuthorize("hasRole(...)")` dan pengecekan role programatik) — menambahkan role kustom yang hak aksesnya dibaca dari database saat runtime adalah perubahan model keamanan yang menyentuh banyak halaman admin sekaligus, bukan penambahan kecil; perlu desain teknis terpisah sebelum implementasi.
- **Risiko tumpang tindih dengan izin per-rapat**: requirement role kustom (US-11) harus dijaga tetap terpisah dari izin granular per-rapat (penunjukan notulis, US-5) yang sengaja *tidak* dimodelkan sebagai role — tanpa batas yang jelas, kedua mekanisme hak akses ini berisiko tumpang tindih atau membingungkan Admin saat dikonfigurasi.
- **Risiko skala flag "auto-invite ke semua rapat" (US-11)**: karena flag ini kini bisa di-assign ke role kustom mana pun (bukan lagi eksklusif satu role kecil seperti Direktur), Admin yang ceroboh meng-assign-nya ke role beranggota banyak bisa membuat setiap rapat baru punya sangat banyak peserta otomatis — tidak ada lagi batasan implisit "hanya satu role/skala kecil yang auto-invite" seperti sekarang. Perlu pertimbangan UX (mis. peringatan jumlah anggota di form Admin) saat implementasi.
