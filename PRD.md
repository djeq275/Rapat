...
- AC: Role kustom **tidak** otomatis mendapat perilaku bawaan yang saat ini di-hardcode ke role tertentu (mis. auto-invite ke semua rapat seperti Direktur, atau menjadi organizer rapat divisi seperti Ketua Divisi) — perilaku semacam itu tetap eksklusif milik role bawaan yang bersangkutan, kecuali diputuskan lain saat desain teknis.
- AC: User dengan role kustom yang tidak diberi hak akses apa pun hanya bisa mengakses halaman yang memang terbuka untuk semua user login (analog dengan Karyawan tanpa divisi hasil auto-provisioning Keycloak, lihat US-1).
...
- **Role kustom & hak akses (US-11)**: 4 role bawaan (Admin, Direktur, Ketua Divisi, Karyawan) tetap ada sebagai baris "terlindungi" yang tidak bisa dihapus/diubah nama, karena masing-masing punya perilaku yang di-hardcode di kode — Direktur otomatis diundang ke setiap rapat (US-3), Ketua Divisi menjadi satu-satunya role yang bisa jadi organizer rapat divisinya dan dibatasi satu Ketua Divisi per divisi. Fitur ini **menambah** lapisan role kustom di atas 4 role bawaan itu (bukan menggantikan/menggeneralisasi semuanya), dengan hak akses per role kustom berupa daftar kapabilitas/menu yang bisa di-assign Admin. Ini menggeser sebagian pemeriksaan akses dari role hardcode di kode (`@PreAuthorize("hasRole(...)")`) menjadi hak akses yang dibaca dari database saat runtime untuk role kustom — role bawaan tetap boleh memakai pemeriksaan hardcode yang sudah ada untuk perilaku khususnya.
